...
export default function FarmerPortal({ onSwitch, onLogout }) {
  ...
  const { user } = useAuth();
  // products is already scoped server-side to the authenticated farmer's own
  // listings when ProductProvider scope="mine". No frontend filter needed.
  const farmerListings = products;
  ...
  // Extract farmer's state from user's address
  const farmerState = getFarmerState(user);
  ...
  // Extract State from "City, State" address string
  function getFarmerState(user) {
