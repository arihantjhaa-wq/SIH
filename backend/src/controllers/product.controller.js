...
// Developer identity is authoritative from the DB flag set only by the
// keyed developer-access flow. Username is never checked here — relying on
// it was the original privilege-escalation vector.
function isDeveloperUser(user) {
  return !!user && user.isDeveloper === true;
}

// True when the authenticated user owns the product, or is a developer
// (developer/admin may manage any product).
function canManageProduct(user, product) {
  if (isDeveloperUser(user)) return true;
  return !!product.addedBy && product.addedBy.toString() === user._id.toString();
}
...
// Authenticated farmer's OWN products only — scoped by the user id from the
// JWT/session. The client can never ask for another user's items here.
const getMyProducts = asyncHandler(async (req, res) => {
  const products = await Product.find({ addedBy: req.user._id }).sort({ createdAt: 1 });

  return res
    .status(200)
    .json(new ApiResponse(200, products, "Your products fetched successfully"));
});

// Developer/Admin only — ALL products with owner info. Rejects non-developers
// regardless of what the client sends.
const getAdminProducts = asyncHandler(async (req, res) => {
  if (!isDeveloperUser(req.user)) {
    throw new ApiError(403, "Not authorized to view all products");
  }
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  // Ownership enforcement: a farmer may only edit their own listing.
  if (!canManageProduct(req.user, product)) {
    throw new ApiError(404, "Product not found");
  }

  const { name, category, unit, indivPrice, bizPrice, minBulkQty } = req.body;

  if (name !== undefined) product.name = name;
  if (category !== undefined) product.category = category;
  if (unit !== undefined) product.unit = unit;
  if (indivPrice !== undefined) product.indivPrice = indivPrice;
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  // Ownership enforcement: a farmer may only delete their own listing;
