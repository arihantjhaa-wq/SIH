...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const { email, username, password, name, address, fullName } = req.body;

  // name is accepted as an alias for fullName (frontend sends `name`)
  const resolvedFullName = fullName || name || undefined;

  // Reserve the developer username — developer status must only ever be
  // granted by the keyed developer-access flow, never by registering it.
  if (username === "__developer__") {
    throw new ApiError(409, "That username is not available");
  }
  ...
  if (resolvedFullName) userCreatePayload.fullName = String(resolvedFullName).trim();
  if (address) userCreatePayload.address = String(address).trim();

  const user = await User.create(userCreatePayload);
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  // isDeveloper now comes from the DB flag set by developer-access only.
  user.isDeveloper = user.isDeveloper === true;
  return res
    .status(200)
...
const developerAccess = asyncHandler(async (req, res) => {
  ...
  } else if (!devUser.isDeveloper) {
    // Existing user created before the field — promote it.
    devUser.isDeveloper = true;
