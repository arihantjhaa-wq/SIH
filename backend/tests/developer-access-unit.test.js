...
// ============================================================================
// Test 7: GST Response Helpers
// ============================================================================

describe("GST Response Helpers", () => {
  let notVerifiedResult, verificationDisabledResult, formatInvalidResult;

  before(async () => {
    const mod = await import("../src/utils/gst-classifier.js");
    notVerifiedResult = mod.notVerifiedResult;
    verificationDisabledResult = mod.verificationDisabledResult;
    formatInvalidResult = mod.formatInvalidResult;
  });

  test("verificationDisabledResult returns correct shape", () => {
    const result = verificationDisabledResult();
    assert.equal(result.status, "VERIFICATION_DISABLED");
    assert.equal(result.data, null);
  });

  test("FORMAT_INVALID for empty input", () => {
    const result = formatInvalidResult("Please enter a valid 15-character GSTIN.");
    assert.equal(result.status, "FORMAT_INVALID");
    assert.equal(result.data, null);
  });

  test("NOT_VERIFIED (real verification disabled — API removed)", () => {
...
// ============================================================================
// Test 8: Developer Demo GSTIN Path (any authenticated user)
// ============================================================================
...
describe("Developer Demo GSTIN Path", () => {
  ...
  test("Demo GSTIN does not reach external API (API removed)", () => {
    const cleanGstin = DEMO_DEVELOPER_GSTIN;
    assert.equal(cleanGstin === DEMO_DEVELOPER_GSTIN, true, "Demo GSTIN matches constant");
  });

  test("Demo verification works for any authenticated user (not just __developer__)", () => {
    // Updated: any authenticated user can use the demo GSTIN
    const currentUser = { username: "regularuser" };
    const isAuthenticated = Boolean(currentUser);
    const cleanGstin = DEMO_DEVELOPER_GSTIN;

    const wouldReturnDemo = isAuthenticated && cleanGstin === DEMO_DEVELOPER_GSTIN;
    assert.equal(wouldReturnDemo, true, "Any authenticated user + demo GSTIN should return demo");
  });

  test("Developer user also gets demo", () => {
    const currentUser = { username: "__developer__" };
    const isAuthenticated = Boolean(currentUser);
    const cleanGstin = DEMO_DEVELOPER_GSTIN;

    const wouldReturnDemo = isAuthenticated && cleanGstin === DEMO_DEVELOPER_GSTIN;
    assert.equal(wouldReturnDemo, true, "Developer + demo GSTIN should return demo");
  });
